...
from config import USERNAME, PASSWORD, PAN, APPID, SECRETID

def generate_token_url(app_id, secret_id, driver):
    ...
    #dob_selection = driver.find_element_by_id('Dobcheck')
    #driver.execute_script("arguments[0].click();", dob_selection)
    pan_card = driver.find_element_by_id('pancard')
    submit_button = driver.find_element_by_id('btn_id')
    # Fill in the credentials
    user_name.send_keys(USERNAME)
    password.send_keys(PASSWORD)
    pan_card.send_keys(PAN)
    submit_button.click()
...
def extract_token(full_url):
    ...
    """
    access_token = re.search(r'(?<=https://www.gmail.com/\?access_token=).*?(?=user_id=' + USERNAME + ')', full_url).group(0)
    if access_token:
...
def main():
    ...
    full_url = generate_token_url(APPID, SECRETID, driver)
